 ...
 * - ready:      Upload confirmed, processing variants (transitional state)
 * - processed:  Variants generated, image fully available for use
 * - failed:     Upload or processing did not complete
 */
export const ImageUploadStatusSchema = z.enum(['pending', 'ready', 'processed', 'failed']);
export type ImageUploadStatus = z.infer<typeof ImageUploadStatusSchema>;

/**
 * Resized/optimised WebP variants generated by the image processing function.
 *
 * - thumbnail: 400px-wide WebP for image picker grids (2× the ~200px display slot)
 * - display:   1000px-wide WebP for audience-facing headers (2× the ~500px max container)
 */
export const ImageVariantsSchema = z.object({
  thumbnail: z.string().url().describe('400px-wide WebP variant for image picker grids'),
  display: z.string().url().describe('1000px-wide WebP variant for audience-facing display'),
});
...
export const ImageSchema = z.object({
  ...
  variants: ImageVariantsSchema.optional().describe('Resized WebP variants (set by image processing function)'),
